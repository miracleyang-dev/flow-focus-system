...
- 🚀 **一键启动**：专为 Windows 用户提供 `.bat` 傻瓜式一键启动脚本，免去繁琐的命令行操作。
...
**方式一：Windows 一键启动（推荐）**
只需双击运行目录下的 `启动心流.bat` 脚本即可。它会自动启动 Node 服务器并在默认浏览器中为你打开应用 (`http://localhost:3000`)。
*(注：本地运行需确保 Redis 已启动并放行默认 6379 端口)*
